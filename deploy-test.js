...
const config = {
    ...
    excludeFiles: [
        ...
        'logs',               // Exclude logs directory (alternative pattern)
        '*/logs/',            // Exclude logs directories in subdirectories
        '*/logs',             // Exclude logs directories in subdirectories (alternative)
        'logs/*.log',         // Exclude individual log files
        'logs/*.txt',         // Exclude text log files
        '*.log',              // Exclude any log files
        'vendor/bin',
        '*.tmp',
        '*.cache'
    ]
...
class TestDeployer {
    ...
    shouldExclude(filename) {
        return config.excludeFiles.some(pattern => {
            ...
            if (pattern.endsWith('/')) {
                return filename === pattern.slice(0, -1) || filename.startsWith(pattern);
            }
            ...
            // Handle subdirectory patterns (*/logs/)
            if (pattern.startsWith('*/')) {
                const subPattern = pattern.slice(2);
                if (subPattern.endsWith('/')) {
                    return filename === subPattern.slice(0, -1);
                }
