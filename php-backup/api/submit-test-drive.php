...
try {
    ...
    $date = date('Y-m-d', strtotime(trim($input['date'] ?? '')));
    ...
    if (empty($date)) {
        ...
        $selectedDate = new DateTime($date, new DateTimeZone('Asia/Kolkata'));
        $tomorrow = new DateTime('now', new DateTimeZone('Asia/Kolkata'));
        $tomorrow->modify('+1 day');
        $tomorrow->setTime(0, 0, 0);
        $selectedDate->setTime(0, 0, 0);

        if ($selectedDate < $tomorrow) {
            $errors[] = 'Please select a future date (tomorrow or later)';
        }
    }
    ...
    if (!empty($errors)) {
        ...
        if (empty($date)) {
            $field_errors['date'] = $errors[$error_index] ?? 'This field is required';
            $error_index++;
        } else {
            $selectedDate = new DateTime($date, new DateTimeZone('Asia/Kolkata'));
            $tomorrow = new DateTime('now', new DateTimeZone('Asia/Kolkata'));
            ...
            if ($selectedDate < $tomorrow) {
                $field_errors['date'] = $errors[$error_index] ?? 'Please select a future date (tomorrow or later)';
                $error_index++;
            }
        }
        ...
        $result = $db->requestTestDrive($full_name, $phone, $date, $pincode, $message, $email);
        ...
        if ($result !== false) {
            ...
            $html_message = (function ($test_ride_id, $full_name, $phone, $email, $date, $pincode, $message, $emailConfig) {
                ob_start();
                include __DIR__ . '/../email-templates/test-ride-admin-email.tpl.php';
                return ob_get_clean();
            })($test_ride_id, $full_name, $phone, $email, $date, $pincode, $message, $emailConfig);
            ...
            $customer_message = (function ($full_name, $test_ride_id, $date, $pincode, $emailConfig, $image_url = 'https://test.kineticev.in/-/images/new/black/000044.png') {
                // Ensure $date is always set for the template
                ...
                return ob_get_clean();
            })($full_name, $test_ride_id, $date, $pincode, $emailConfig);
...
Preferred Date: " . date('d M Y', strtotime($date)) . "
Location: Pin Code {$pincode}

{$emailConfig['showroom_contact']}

Our team will contact you within {$emailConfig['response_time']} to confirm the test ride details and location.
...
KineticEV Test Ride Team
            ...
            $response['message'] = $overallEmailSuccess
                ? "Thank you for your test ride request! We have sent a confirmation email and our team will contact you within {$emailConfig['response_time']} to arrange your test ride."
                : "Thank you for your test ride request! Our team will contact you within {$emailConfig['response_time']} to arrange your test ride. (Note: Email delivery may be delayed)";
